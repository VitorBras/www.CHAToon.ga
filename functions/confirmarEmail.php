...
//Iniciar sessão
//Caso PHPSESSID exista
if(isset($_REQUEST['PHPSESSID']) == true){//Independente de existir

	//Autenticação via email 
	if(isset($_REQUEST['throwEmail']) == true and isset($_REQUEST['PHPSESSID']) == true){
		session_id($_REQUEST['PHPSESSID']);
		session_start();
	}else{
		//Autenticação via inserção de código na WebApp (Acho que o Cookia é repassado na REQUISIÇÃO AJAX)
		session_id($_REQUEST['PHPSESSID']);
		session_start();
	}
			...
			if(isset($_REQUEST['confirmCode'])){
				...
				if(isset($_SESSION['hbname']) AND isset($_SESSION['hbserver'])){//Dados condizem
					...
					if($infos != false){
						...
						$sending = sendEmail("email_confirm_code",$infos['email'],$hbname,$hbserver,$infos['codigo_email'],$link,session_id());
